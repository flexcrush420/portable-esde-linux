...
github_appimage() {
    ...
    url=$(printf '%s' "$resp" \
        ...
        | grep -P "$pattern" \
        | grep -ivE 'aarch|arm64|armv7|zsync|sha256|dbg|\.sig$|\.asc$' \
        | head -1) || true
    if [[ -z "$url" ]]; then
        fail "No matching asset for pattern in $repo releases (release exists, asset does not)"
        return 1
    fi
    # If the resolved versioned asset is ALREADY present, don't re-download it
    # under the stable -latest name. On first install we save as
    # <name>-latest.AppImage; update.sh later renames that to the real release
    # name. Without this check, a setup re-run no longer sees the -latest file,
    # re-downloads it, and leaves a stale <name>-latest.AppImage duplicate
    # beside the versioned one. Matching the real basename also sidesteps
    # prefix/case naming mismatches (Dolphin_Emulator vs dolphin-emu, etc.).
    if [[ -f "$(dirname "$outfile")/$(basename "$url")" ]]; then
        ok "$(basename "$url") already present, skipping"
        return 0
    fi
...
install_ryujinx() {
    # Ryubing (Ryujinx fork) is hosted on a self-managed Forgejo (Gitea)
    # instance at git.ryujinx.app. It was DMCA'd off GitHub, so there is no
    # GitHub mirror to fall back to.
    if compgen -G "$EMUS/ryujinx*.AppImage" > /dev/null 2>&1; then
        ok "Ryubing already exists, skipping"
    else
        info "Downloading Ryubing ..."
        # Query the Forgejo (Gitea) API directly. The previous GitHub-first
        # attempt only wasted a rate-limited api.github.com request against the
        # DMCA-blocked repo before falling back to exactly this endpoint.
        RYUBING_URL=$(curl -sfL --max-time 20 "https://git.ryujinx.app/api/v1/repos/ryubing/ryujinx/releases?limit=5" \
            | grep -oP '"browser_download_url":\s*"\K[^"]*' \
            | grep -P "x64\.AppImage$" \
            | grep -ivE 'arm|aarch' \
            | head -1) || true
...
# Verify a downloaded file is a real ELF/AppImage BEFORE it is allowed to
# replace a working binary. Guards against rate-limit HTML, source zips, and
# wrong-platform builds (e.g. the ES-DE Windows portable ZIP, magic 504b0304)
# being written over a good AppImage. Returns 0 only for ELF magic 7f454c46.
is_elf() {
    [[ -s "$1" ]] || return 1
...
github_latest_url() {
    # Args: repo  pattern  [exclude_regex]
    # The optional exclude regex (e.g. 'aarch|arm') is applied to the FULL
    # asset list before head -1. Piping a filter AFTER this function (the old
    # call sites did ` | grep -ivE 'aarch|arm'`) only ever operated on the
    # single already-chosen line, so it could blank the result to empty
    # ("could not check") or let a wrong-arch asset through as "up to date"
    # (Supermodel picked an aarch64 build this way). Filter before choosing.
    local repo="$1" pattern="$2" exclude="${3:-}"
    # Trailing `|| true`: when grep filters every candidate (no match, or the
    # exclude removes all), the pipeline would return non-zero under
    # `set -o pipefail`. Callers only ever consume the OUTPUT (empty ==
    # "could not check"), never the exit code, so normalise to success to stay
    # safe in any calling context (argument or assignment).
...
check_and_update() {
    ...
    local current_file
    # -iname (case-insensitive): the installed Dolphin is Dolphin_Emulator_*
    # but the glob was dolphin* — a case-sensitive -name missed it and reported
    # "not installed". Case-insensitive matching fixes that class of miss.
    current_file=$(find "$EMUS" -maxdepth 2 -type f -iname "$current_glob" 2>/dev/null | head -1)
    [[ -z "$current_file" ]] && { warn "$label — not installed"; return; }

    local current_name latest_name dest_dir
    current_name=$(basename "$current_file")
    latest_name=$(basename "$latest_url")
    # Download BESIDE the file being replaced. For ports the current AppImage
    # lives in its own Emulators/<Port>/ folder (the launcher runs the engine
    # from there so beside-the-binary game data is found); downloading to the
    # shared Emulators/ root instead hoisted it out of that folder. dirname
    # keeps ports foldered and is a no-op for top-level emulators.
    dest_dir=$(dirname "$current_file")
    printf "   %-32s" "$label"
    ...
    if [[ "$current_name" == "$latest_name" ]]; then
        echo -e " ${GREEN}[up to date]${NC}  $current_name"
        # Clean stale duplicates even when no download happens — e.g. a setup
        # re-run recreated <Emu>-latest.AppImage beside the versioned file a
        # prior update had left. Keep the current match; -type f means a port
        # data directory is never removed, and the glob is emulator-specific.
        find "$dest_dir" -maxdepth 1 -type f -iname "$current_glob" \
            ! -name "$current_name" -delete 2>/dev/null || true
        return
    fi
    ...
    local _msg
    _msg=$(printf 'Update %s?\n\n  Current:  %s\n  Latest:   %s' "$label" "$current_name" "$latest_name")
    if wt_yesno "Update: $label" "$_msg"; then
        info "Downloading $latest_name ..."
        local tmp="$dest_dir/$latest_name.tmp.$$"
        if curl -#fL -o "$tmp" "$latest_url"; then
            if ! is_elf "$tmp"; then
                warn "Downloaded file is not an AppImage (bad/blocked download) — keeping current version"
                rm -f "$tmp"; echo ""; return
            fi
            mv -f "$tmp" "$dest_dir/$latest_name"
            chmod +x "$dest_dir/$latest_name"
            # Remove any OTHER stale versions of THIS emulator in the same
            # directory. -type f guarantees we never touch a port's data
            # DIRECTORY (e.g. Emulators/OpenTTD/); the glob is emulator-specific
            # so unrelated files are untouched; excluding the new name keeps the
            # copy we just placed.
            find "$dest_dir" -maxdepth 1 -type f -iname "$current_glob" \
                ! -name "$latest_name" -delete 2>/dev/null || true
...
# ── ES-DE ──
# ES-DE is distributed on GitLab. Resolve the URL from the GitLab release API
# and pick the asset by NAME (reliably "ES-DE_x64.AppImage"), excluding
# Windows/macOS/Steam Deck/ARM assets. The previous version scraped
# https://es-de.org/ and took the FIRST package_files/<id>/download link — but
# those numeric URLs are platform-indistinguishable and the Windows portable
# ZIP is listed first, so it silently overwrote the AppImage with a Windows
# build (avcodec-*.dll, magic 504b0304). Every download is now ELF-validated in
# a temp file BEFORE it may replace the working AppImage.
echo -e "${CYAN}ES-DE Frontend:${NC}"
ESDE_CURRENT=$(find "$SCRIPT_DIR" -maxdepth 1 -name 'ES-DE*.AppImage' | head -1)
if [[ -n "$ESDE_CURRENT" ]]; then
    printf "   %-32s" "ES-DE"
    ESDE_API="https://gitlab.com/api/v4/projects/es-de%2Femulationstation-de/releases/permalink/latest"
    ESDE_JSON=$(curl -sfL "$ESDE_API") || true
    ESDE_URL=$(printf '%s' "$ESDE_JSON" | python3 -c '
import sys, json
try: d = json.load(sys.stdin)
except Exception: sys.exit(0)
links = (d.get("assets") or {}).get("links") or []
def u(l): return l.get("direct_asset_url") or l.get("url") or ""
def s(l): return ((l.get("name") or "") + " " + u(l)).lower()
def bad(t): return any(x in t for x in ["steamdeck","aarch64","arm64","armhf","win","macos",".dmg",".exe",".zsync",".sha",".sig",".asc",".deb",".rpm",".tar"])
for l in links:
    t = s(l)
    if ".appimage" in t and ("x64" in t or "x86_64" in t or "x86-64" in t) and not bad(t): print(u(l)); sys.exit(0)
for l in links:
...
') || true
    if [[ -n "$ESDE_URL" ]]; then
        echo -e " ${CYAN}[latest resolved from GitLab]${NC}"
        echo -e "      Installed: $(basename "$ESDE_CURRENT")"
        if wt_yesno "Update: ES-DE" "Re-download the latest ES-DE AppImage from GitLab?"; then
            ESDE_TMP="$SCRIPT_DIR/ES-DE_x64.AppImage.tmp.$$"
            if curl -#fL -o "$ESDE_TMP" "$ESDE_URL"; then
                if is_elf "$ESDE_TMP"; then
                    chmod +x "$ESDE_TMP"
                    mv -f "$ESDE_TMP" "$SCRIPT_DIR/ES-DE_x64.AppImage"
                    [[ "$ESDE_CURRENT" != "$SCRIPT_DIR/ES-DE_x64.AppImage" ]] && rm -f "$ESDE_CURRENT"
                    ok "ES-DE updated"
                else
                    warn "Downloaded file is not an ELF AppImage (got $(file -b "$ESDE_TMP" 2>/dev/null | cut -c1-40)) — keeping current version"
                    rm -f "$ESDE_TMP"
                fi
            else
                warn "Download failed — keeping current version"; rm -f "$ESDE_TMP"
            fi
        fi
    else
        echo -e " ${YELLOW}[could not resolve GitLab AppImage URL]${NC}"
    fi
fi
echo ""
...
# RPCS3 (redirect URL, always latest). Download into a CLEAN temp dir so
# curl -J names the file from the server without ever colliding with the
# installed copy — the old `(cd "$EMUS" && curl -fJLO …)` aborted with
# "<name>: File exists" whenever the server offered the same filename already
# present (curl -J refuses to overwrite), which is why a re-run reported
# "Download failed" even though nothing was wrong. Validate ELF, then swap.
#
# Recovery: if the only RPCS3 left is an orphaned "*.AppImage_old" (a rename
# left behind by an interrupted run — the glob 'rpcs3*.AppImage' can't see it,
# so RPCS3 silently vanished from the update list), restore it first so there
# is always a working binary even offline. Gate on the launcher OR any rpcs3
# file so a missing/renamed AppImage still offers a re-download instead of
# being skipped.
RPCS3_CURRENT=$(find "$EMUS" -maxdepth 1 -type f -iname 'rpcs3*.AppImage' | head -1)
if [[ -z "$RPCS3_CURRENT" ]]; then
    RPCS3_ORPHAN=$(find "$EMUS" -maxdepth 1 -type f -iname 'rpcs3*.AppImage_old' | head -1)
    if [[ -n "$RPCS3_ORPHAN" ]]; then
        mv -f "$RPCS3_ORPHAN" "${RPCS3_ORPHAN%_old}"
        chmod +x "${RPCS3_ORPHAN%_old}" 2>/dev/null || true
        RPCS3_CURRENT="${RPCS3_ORPHAN%_old}"
        warn "RPCS3: restored orphaned $(basename "${RPCS3_ORPHAN%_old}") (no active AppImage was present)"
    fi
fi
if [[ -n "$RPCS3_CURRENT" || -f "$EMUS/rpcs3-portable.sh" ]]; then
    printf "   %-32s" "RPCS3 (PS3)"
    echo -e " ${CYAN}[nightly — always latest when re-downloaded]${NC}"
    if wt_yesno "Update: RPCS3" "Re-download the latest RPCS3 nightly build?"; then
        RPCS3_TMP=$(mktemp -d)
        if (cd "$RPCS3_TMP" && curl -#fJLO "https://rpcs3.net/latest-linux-x64"); then
            RPCS3_DL=$(find "$RPCS3_TMP" -maxdepth 1 -type f -iname 'rpcs3*.AppImage' | head -1)
            if [[ -n "$RPCS3_DL" ]] && is_elf "$RPCS3_DL"; then
                chmod +x "$RPCS3_DL"
                # Sweep stale actives AND any leftover *_old orphans.
                find "$EMUS" -maxdepth 1 -type f \
                    \( -iname 'rpcs3*.AppImage' -o -iname 'rpcs3*.AppImage_old' \) -delete
                mv -f "$RPCS3_DL" "$EMUS/$(basename "$RPCS3_DL")"
                ok "RPCS3 updated"
            else
                warn "Downloaded file is not an AppImage — keeping current version"
            fi
        else
            warn "Download failed — keeping current version"
        fi
...
if [[ -n "$RA_CURRENT" ]]; then
    ...
    echo -e " ${CYAN}[nightly — always latest when re-downloaded]${NC}"
    if wt_yesno "Update: RetroArch" "Re-download the latest RetroArch nightly AppImage?"; then
...
# Match ONLY the regular SSE4.1 build (DuckStation-x64.AppImage), never the
# legacy DuckStation-x64-SSE2.AppImage fallback. The old 'DuckStation.*x64.*'
# pattern also matched the -SSE2 asset, and head -1 grabbed it — downgrading
# capable CPUs to the slow build and tripping DuckStation's own SSE2 hardware
# warning. The exact end-anchor excludes -SSE2; the 'SSE2' exclude is belt-and-
# suspenders in case the pattern is ever loosened.
check_and_update "DuckStation (PS1)" "DuckStation*.AppImage" \
    "$(github_latest_url stenzek/duckstation 'DuckStation-x64\.AppImage$' 'SSE2')"
...
# xemu ships both a release and a '-dbg' debug build per version; the loose
# pattern + head -1 grabbed the slower debug AppImage. Exclude 'dbg'.
check_and_update "xemu (Xbox)" "xemu*.AppImage" \
    "$(github_latest_url xemu-project/xemu 'xemu.*x86_64\.AppImage$' 'dbg')"

# Eden (Yuzu successor) was DMCA'd off GitHub — eden-emulator/Releases now
# returns HTTP 451 ("Repository access blocked", Nintendo, 2026-03-05), so the
# old github_latest_url call silently yielded nothing ("could not check").
# Releases moved to their own Gitea server at git.eden-emu.dev; this mirrors the
# resolver the setup installer already uses (stable first, nightly fallback).
# --max-time avoids a hang; an empty result degrades to "could not check".
EDEN_URL=$(curl -sfL --max-time 20 "https://git.eden-emu.dev/api/v1/repos/eden-emu/eden/releases?limit=5&token=" 2>/dev/null \
    | grep -oP '"browser_download_url":\s*"\K[^"]*' \
    | grep -iP "amd64.*\.AppImage$|x86_64.*\.AppImage$" \
    | grep -iv "arm\|zsync\|deb\|room" \
    | head -1) || true
if [[ -z "$EDEN_URL" ]]; then
    EDEN_URL=$(curl -sfL --max-time 20 "https://git.eden-emu.dev/api/v1/repos/eden-ci/nightly/releases?limit=3" 2>/dev/null \
        | grep -oP '"browser_download_url":\s*"\K[^"]*' \
        | grep -iP "amd64.*\.AppImage$|x86_64.*\.AppImage$" \
        | grep -iv "arm\|zsync\|deb\|room" \
        | head -1) || true
fi
check_and_update "Eden (Switch)" "Eden*.AppImage" "$EDEN_URL"
...
if [[ -n "$SHADPS4_CURRENT" ]]; then
    ...
    if wt_yesno "Update: shadPS4" "Re-download the latest shadPS4 release (binary + support files)?"; then
...
if [[ -f "$VPDIR/VPinballX_BGFX" ]] || [[ -f "$VPDIR/VPinballX_GL" ]]; then
    ...
    echo -e " ${CYAN}[zip release — re-download to update]${NC}"
    if wt_yesno "Update: VPinball" "Re-download the latest VPinball (BGFX + GL) release?"; then
...
check_and_update "OpenTTD" "OpenTTD*.AppImage" \
    "$(github_latest_url pkgforge-dev/OpenTTD-AppImage '\.AppImage$' 'aarch|arm')"
check_and_update "Commander Genius" "Commander*Genius*.AppImage" \
    "$(github_latest_url pkgforge-dev/Commander-Genius-AppImage '\.AppImage$' 'aarch|arm')"
check_and_update "C-Dogs SDL" "C-Dogs_SDL*.AppImage" \
    "$(github_latest_url pkgforge-dev/C-Dogs_SDL-AppImage '\.AppImage$' 'aarch|arm')"
check_and_update "OpenTyrian 2000" "OpenTyrian2000*.AppImage" \
    "$(github_latest_url pkgforge-dev/OpenTyrian2000-AppImage '\.AppImage$' 'aarch|arm')"
check_and_update "OpenRA" "OpenRA*.AppImage" \
    "$(github_latest_url pkgforge-dev/OpenRA-AppImage-Enhanced '\.AppImage$' 'aarch|arm')"

# Supermodel
check_and_update "Supermodel (Model 3)" "Supermodel*.AppImage"     "$(github_latest_url pkgforge-dev/Supermodel-AppImage '\.AppImage$' 'aarch|arm')"

# Ruffle — tar.gz binary
if [[ -f "$EMUS/ruffle" ]]; then
    printf "   %-32s" "Ruffle (Flash)"
    echo -e " ${CYAN}[tar.gz release — re-download to update]${NC}"
    if wt_yesno "Update: Ruffle" "Re-download the latest Ruffle release?"; then
        RUFFLE_URL=$(curl -sfL "https://api.github.com/repos/ruffle-rs/ruffle/releases?per_page=3"             | grep -oP '"browser_download_url":\s*"\K[^"]*'             | grep -iP "linux.*x86.?64.*\.tar\.gz$" | grep -iv "debug\|arm" | head -1) || true
        if [[ -n "$RUFFLE_URL" ]]; then
            RUFFLE_TMP=$(mktemp -d)
            if curl -#fL "$RUFFLE_URL" | tar -xz -C "$RUFFLE_TMP" 2>/dev/null; then
...
# Ryubing (Ryujinx fork) left GitHub — releases now live on their own Forgejo
# server at git.ryujinx.app, and GitHub update support was removed upstream, so
# the old api.github.com/repos/Ryubing/Ryujinx call always returned nothing
# ("could not check"). Query the Forgejo (Gitea-compatible) API instead. Best
# effort: --max-time keeps a slow/unreachable host from hanging the updater,
# and an empty result degrades to the normal "could not check" line.
RYUBING_URL=$(curl -sfL --max-time 20 \
    "https://git.ryujinx.app/api/v1/repos/ryubing/ryujinx/releases/latest" 2>/dev/null \
    | grep -oP '"browser_download_url":\s*"\K[^"]*' \
    | grep -P 'x64\.AppImage$' \
    | grep -ivE 'arm|aarch' \
    | head -1) || true
check_and_update "Ryubing (Switch)" "ryujinx*.AppImage" "$RYUBING_URL"
