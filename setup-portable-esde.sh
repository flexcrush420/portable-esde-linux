...
# STEP 4b populates this directory from the libretro repo.
...
# libSDL3_ttf.so.0 and libfreeimage.so. SDL3 is not system-wide on any
...
# VPX reads VPinballX.ini from $HOME/.vpinball/. With HOME set to the
# bundle root by the parent launch.sh, that resolves to
# $BASE_DIR/.vpinball/VPinballX.ini. The critical key is PinMAMEPath:
# without it pointing at our bundle's pinmame folder, PinMAME finds zero
# ROMs at default lookup, every SS table crashes on coin press, and the
# log shows misleading B2S Server.ChangedPDLeds dispatch errors as the
# downstream symptom. We refresh the ini every launch so the bundle
# stays portable across drive moves and Linux reinstalls (absolute path
# inside ini gets re-stamped to current SCRIPT_DIR each time).
VPI_DIR="$BASE_DIR/.vpinball"
VPI="$VPI_DIR/VPinballX.ini"
PINMAME_PATH="$BASE_DIR/ROMs/vpinball/pinmame"
mkdir -p "$VPI_DIR"
if [[ ! -f "$VPI" ]]; then
    # No ini yet: write a minimal one with the keys we manage. VPX will
    # fill in everything else with its own defaults on first launch.
    cat > "$VPI" << EOF
[Standalone]
[Plugin.PinMAME]
...
EOF
    ...
    # Ini exists (user may have edited it). Touch only PinMAMEPath:
    # rewrite if present, append under [Plugin.PinMAME] if section exists,
    # otherwise add the whole section at the end. Other keys left alone.
    if grep -qE "^\s*PinMAMEPath\s*=" "$VPI"; then
        sed -i "s|^\s*PinMAMEPath\s*=.*|PinMAMEPath = $PINMAME_PATH|" "$VPI"
    elif grep -qE "^\s*\[Plugin\.PinMAME\]" "$VPI"; then
        sed -i "/^\s*\[Plugin\.PinMAME\]/a PinMAMEPath = $PINMAME_PATH" "$VPI"
    else
        printf '\n[Plugin.PinMAME]\nPinMAMEPath = %s\n' "$PINMAME_PATH" >> "$VPI"
    fi
fi
...
All configured for fullscreen and portable paths out of the box. Set per-emulator video preferences (resolution, render settings) in each emulator's own settings menu on first launch — the bundle preserves those choices on every setup re-run.
...
- **Where to install** (defaults to `./ES-DE` in the current directory)
