...
# Seed the bundled default theme on the FIRST launch only, then step aside.
# ES-DE saves the user's theme choice to es_settings.xml on exit, so once the
# bundle has launched once we must NOT keep re-forcing the cached theme — doing
# so means a user can never switch themes and have the change stick. apply_theme
# therefore CONSUMES the cache: it applies the desired theme once, then deletes
# the cache file so every later launch leaves the user's selection untouched.
...
# Re-heal bundle paths after exit (ES-DE may rewrite some settings on quit).
# Deliberately NOT re-applying the theme here: ES-DE has just saved the user's
# theme choice on exit, and re-forcing the cached theme would overwrite it.
apply_paths
...
if [[ -n "${THEME_SKIP_REASON:-}" ]]; then
    ...
    if [[ -n "$THEME_NAME" ]]; then
        if [[ -f "$ESDE_DATA/.desired_theme" ]]; then
            ...
            ok "Theme preference saved (applied on first launch, then user-switchable)"
        fi
    fi
else
