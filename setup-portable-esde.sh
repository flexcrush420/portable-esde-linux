...
<systemList>
  ...
  <system>
    ...
    <command label="Launch">%ROM%</command>
    <platform>pc</platform>
    <theme>ports</theme>
  </system>
...
for engine_key in devilutionx corsixth cgenius cdogs eduke32 ghostship gzdoom openbor openjazz opentyrian openrct2 openloco openra xash3d yquake2 ioquake3 dhewm3; do
    if emu_selected "$engine_key"; then
        install_engine_by_key "$engine_key"
    else
...
looks_like_generic_bios_archive() {
    ...
    # Old check used unanchored *bios* / *firmware* / *system* / *kickstart*
    # substring globs, which misrouted real games into ROMs/bios and extracted
    # them — e.g. "Battlezone (System 16).zip", "Kickstart (homebrew).zip",
    # "Sound System.zip", anything with "firmware" in its release group tag.
    # New rule: the basename (sans extension) must EITHER be exactly one of
    # the keywords, OR start with a keyword followed by '-', '_', or '.'.
    # That catches the real shapes (bios.zip, firmware.rar, system.7z,
    # kickstart.zip, bios-eu.zip, firmware_v1.zip, bios.eu.zip) and rejects
    # game titles that just happen to contain the word — including
    # parenthesised forms like "Kickstart (homebrew).zip" where a space
    # follows the keyword. A pure-name BIOS bundle with a space (e.g.
    # "system bios.zip") will not match; such files are rare and known
    # ones are already covered by is_known_bios_file via BIOS_TABLE.
    [[ "$lower" =~ ^(bios|firmware|system|kickstart)([._-].*)?\.(zip|7z|rar)$ ]]
}
