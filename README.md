...
```bash
chmod +x setup-portable-esde.sh && ./setup-portable-esde.sh
# Add ROMs to ROMs/<system>/  |  BIOS files to ROMs/bios/
./launch.sh
```

## Emulators included

**RetroArch cores** (covers most systems):
NES/Famicom (Mesen) · SNES (Snes9x) · Genesis/Mega Drive (Genesis Plus GX) ·
GB/GBC (Gambatte) · GBA (mGBA) · N64 (Mupen64Plus-Next) · PS1 (Mednafen PSX HW) ·
Saturn (Kronos) · PC Engine/TG16 (Mednafen) · Neo Geo/Arcade (FBNeo + MAME) ·
Dreamcast (Flycast) · MSX/MSX2/TurboR (blueMSX) · C64 (VICE x64sc) ·
Amiga (PUAE) · Atari (Stella/Handy/Hatari/Jaguar) · WonderSwan · ZX Spectrum ·
Sharp X68000/X1 · NEC PC-98 · and 20+ more
...
| PPSSPP        | PSP                              |
| xemu          | Xbox                             |
| Xenia Canary  | Xbox 360 / XBLA                  |
| VPinball      | Visual Pinball (.vpx)            |
| DOSBox-X      | DOS games                        |
| 86Box         | Win98 / Windows 9x / retro PC    |
| Ruffle        | Adobe Flash (.swf)               |
| Solarus       | Solarus engine games             |
| SimCoupe      | MGT SAM Coupé                    |
| Supermodel    | Sega Model 3                     |

**PICO-8:** commercial software — purchase at https://lexaloffle.com/pico-8.php
then place the `pico8` binary in `Emulators/`. The free `fake08` core works as fallback.

## Adding ROMs

Place ROMs in the matching subfolder under `ROMs/`. Run `./convert-retrobat.sh` to
import from an existing RetroBat collection (handles media and gamelists too).

## Updating

Run `./update.sh` to fetch latest AppImage versions and ES-DE releases.
Existing files are skipped unless you confirm re-download. RetroArch cores
update from buildbot automatically.

## Portability notes

The bundle is fully self-contained — move or copy it anywhere.
Paths are re-applied on every launch via `launch.sh`.

## FUSE / libfuse2

AppImages require libfuse2 on the host system:
- Ubuntu/Debian: `sudo apt install libfuse2`
- Fedora:        `sudo dnf install fuse-libs`
- Arch:          `sudo pacman -S fuse2`# Portable ES-DE for Linux
